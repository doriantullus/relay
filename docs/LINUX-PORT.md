...
**Known pre-existing failure**, unrelated to this plan: on arm64 Linux,
`queue.engine`'s "progress events are coalesced far below chunk count"
(`src/core/queue/engine.zig:1134`, `try testing.expect(progress * 4 < reads)`)
fails consistently — 326 pass / 7 skip / 1 fail. It is timing-sensitive
(`progress_interval_ms = 4` against a 0.5 ms-per-chunk mock stall), and CI only
exercises amd64, so it may be arm64-specific. Worth settling before step 1, so
"both green" means something.
...
emulation. Only the GTK layer needs a Linux box.
